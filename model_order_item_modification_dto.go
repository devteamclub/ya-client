...
type OrderItemModificationDTO struct {
	// Идентификатор товара в рамках заказа.  Получить идентификатор можно с помощью ресурсов `GET /campaigns/{campaignId}/orders` или `GET /campaigns/{campaignId}/orders/{orderId}`.  Обязательный параметр. 
	Id int64 `json:"id"`
	// Новое количество товара.
	Count int32 `json:"count"`
	// Информация о маркировке единиц товара.  Передавайте в запросе все единицы товара, который подлежит маркировке.  Обязательный параметр, если в заказе есть товары, подлежащие маркировке в системе «Честный ЗНАК». 
...
func (o *OrderItemModificationDTO) UnmarshalJSON(bytes []byte) (err error) {
	...
	err = json.Unmarshal(bytes, &varOrderItemModificationDTO)
