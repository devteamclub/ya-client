...
// ParcelBoxLabelDTO Информация о ярлыке для грузового места.
type ParcelBoxLabelDTO struct {
	...
	// Идентификатор грузового места в информационной системе магазина.  Возвращается в формате: номер заказа на Маркете-номер грузового места. Например, 7206821‑1, 7206821‑2 и т. д. 
	...
	// Устаревший параметр.  Общая масса всех товаров в заказе. Возвращается в формате: 'weight' кг. 
...
func (o *ParcelBoxLabelDTO) UnmarshalJSON(bytes []byte) (err error) {
	...
	err = json.Unmarshal(bytes, &varParcelBoxLabelDTO)
