...
type OrderLabelDTO struct {
	...
	// Количество грузовых мест в заказе.
	PlacesNumber int32 `json:"placesNumber"`
	// URL файла с ярлыками‑наклейками на все грузовые места в заказе.  Соответствует URL, по которому выполняется запрос `GET /campaigns/{campaignId}/orders/{orderId}/delivery/labels`. 
	Url string `json:"url"`
...
func (o *OrderLabelDTO) UnmarshalJSON(bytes []byte) (err error) {
	...
	err = json.Unmarshal(bytes, &varOrderLabelDTO)
