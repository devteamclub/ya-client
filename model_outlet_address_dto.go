...
// OutletAddressDTO Адрес точки продаж. Обязательный параметр. 
type OutletAddressDTO struct {
	// Идентификатор региона. Идентификатор можно получить c помощью запроса `GET /regions`.  {% note alert %}  При создании и редактировании точек продаж можно указывать только регионы типов `TOWN` (город), `CITY` (крупный город) и `REPUBLIC_AREA` (район субъекта федерации). Тип региона указан в выходных параметрах type запросов `GET /regions` и `GET /regions/{regionId}`.  {% endnote %}  Обязательный параметр. 
	...
	// {% note alert %}  Параметр устарел и не рекомендуется к использованию. Город или населенный пункт возвращается в параметре `regionId`.  {% endnote %} 
...
func (o *OutletAddressDTO) UnmarshalJSON(bytes []byte) (err error) {
	...
	err = json.Unmarshal(bytes, &varOutletAddressDTO)
